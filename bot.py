...
# ── منو ──
def main_menu():
    kb = [[KeyboardButton("📸 ارسال عکس صبحانه"), KeyboardButton("📝 ارسال گزارش شب")]]
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # علامت‌گذاری که این کاربر در حال ثبت‌نامه - ذخیره در فایل نه memory
    data["pending"][str(user.id)] = True
    save_data(data)
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(f"TEXT from {user.id}: '{text}' | pending={data['pending'].get(str(user.id))} | registered={str(user.id) in data['students']}")

    # اگه در حال ثبت‌نامه
    if data["pending"].get(str(user.id)) and str(user.id) not in data["students"]:
        if len(text) < 3:
            await update.message.reply_text("اسم و فامیل کامل بنویس (حداقل ۳ حرف):")
            return

        data["students"][str(user.id)] = {"name": text, "username": user.username or "", "joined": get_today()}
...
def questions_kb():
    btns = [InlineKeyboardButton(f"{q}+" if q==QUESTIONS_OPTIONS[-1] else str(q), callback_data=f"q_{q}") for q in QUESTIONS_OPTIONS]
...
async def cmd_students(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for sid, s in students.items(): msg += f"• {s['name']} | @{s.get('username','ندارد')} | از {s['joined']}\n"
    await update.message.reply_text(msg)

async def cmd_ranking(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ADMIN_ID: return
    data = load_data(); totals = weekly_totals(data)
    ranked = sorted(totals.values(), key=lambda t:(t["hours"],t["questions"]), reverse=True)
    medals = ["🥇","🥈","🥉"]
    msg = "🏆 رتبه‌بندی هفتگی\n" + "━"*20 + "\n\n"
    for i,t in enumerate(ranked): msg += f"{medals[i] if i<3 else str(i+1)+'.'} {t['name']}\n   📚 {t['hours']} ساعت  |  ✏️ {t['questions']} سوال\n\n"
    await update.message.reply_text(msg)

async def cmd_weekly(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ADMIN_ID: return
    data = load_data(); totals = weekly_totals(data)
    if not totals: await update.message.reply_text("هنوز دانش‌آموزی ثبت‌نام نکرده."); return
    ...
    ax.set_title("Weekly Report",fontsize=14,fontweight="bold"); ax.legend(); ax.grid(axis="y",linestyle="--",alpha=0.5,zorder=0)
    fig.tight_layout(); path=os.path.join(CHARTS_DIR,"weekly.png"); fig.savefig(path,dpi=150); plt.close(fig)
    with open(path,"rb") as img: await update.message.reply_photo(img,caption="📊 نمودار هفتگی")
...
def main():
    ...
    # متن عمومی (ثبت اسم) - قبل از report_conv
    app.add_handler(MessageHandler(
        filters.TEXT & ~filters.COMMAND & ~filters.Regex("^📝 ارسال گزارش شب$"),
        handle_text
    ))
