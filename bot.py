...
ONBOARDING_MESSAGE = (
    ...
    "📸 ارسال عکس صبحانه → باید بین ساعت ۵:۳۰ تا ۸:۰۰ صبح بفرستی تا به‌موقع حساب بشه. "
    "بعد از ۸ هم میتونی بفرستی ولی با تاخیر ثبت میشه. اگه تا ۸ صبح اصلاً نفرستی، بهت پیام میدم «اعلام بیداری نکردیا 😒»\n"
    ...
    "💬 درد و دل → هرچی تو دلته بنویس، فقط برای خودت می‌مونه\n"
    ...
    "✉️ نامه به آینده → یه نامه بنویس، دقیقاً یه ماه دیگه بهت میدمش\n"
    "⏰ یادم بنداز → یادآوری شخصی برای هر کاری که میخوای بساز\n"
    "🎓 پرسش مستقیم از استاد → سوالتو مستقیم از استاد بپرس\n"
    "✨ توربو جادویی → کارای هفته‌ت رو تعریف کن، دوبار در روز یادت میندازم\n"
    "🏆 رکورد های من → بهترین رکوردهای هفته اخیرت رو ببین\n"
    "🤖 چت با من → هر سوالی داشتی از هوش مصنوعی بپرس\n"
    "🎁 دریافت مشاوره رایگان → لینک اختصاصی بگیر؛ به ازای هر ۱۰ نفر دعوت، یک جلسه مشاوره تلفنی رایگان میگیری\n\n"
...
GUIDE_MESSAGE = (
    ...
    "📸 ارسال عکس صبحانه\n"
    "بین ساعت ۵:۳۰ تا ۸:۰۰ صبح عکس صبحونه‌ت رو بفرست تا به‌موقع حساب بشه. "
    "بعد از ۸ هم قبول میشه ولی با تاخیر ثبت میشه. ساعت ۷:۴۵ اگه هنوز نفرستاده باشی یه یادآوری میگیری، "
    "و اگه تا ۸ صبح کامل بی‌خبر باشی، پیام «اعلام بیداری نکردیا 😒» میاد.\n\n"
    "📝 ارسال گزارش شب\n"
...
def main_menu():
    kb = [
        ...
        [KeyboardButton("🎓 پرسش مستقیم از استاد"), KeyboardButton("✨ توربو جادویی")],
        [KeyboardButton("🏆 رکورد های من"),       KeyboardButton("🤖 چت با من")],
        [KeyboardButton("🎁 دریافت مشاوره رایگان")],
...
async def job_breakfast_reminder(context: ContextTypes.DEFAULT_TYPE):
    """ساعت ۷:۴۵ صبح؛ فقط به کسایی که هنوز عکس صبحانه نفرستادن یادآوری میکنه."""
...
async def job_report_streak_check(context: ContextTypes.DEFAULT_TYPE):
    """اگه استریک عدم‌ارسال گزارش دقیقاً به یکی از روزهای مشخص (۲،۴،۶،۸،۱۰،۱۵) برسه، پیام دلسوزانه میفرسته."""
    data = load_data()
    for sid, sinfo in data["students"].items():
        streak = compute_missing_report_streak(data, sid, sinfo.get("joined"))
        if streak in REPORT_STREAK_MESSAGES:
            try:
                await context.bot.send_message(int(sid), REPORT_STREAK_MESSAGES[streak])
            except Exception as e:
                logger.warning(f"پیام نگرانی گزارش (استریک {streak}) به {sid}: {e}")

async def job_late_breakfast_check(context: ContextTypes.DEFAULT_TYPE):
    data = load_data(); today = get_today()
    for sid in data["students"]:
        rec = data["daily_records"].get(today, {}).get(sid, {})
        if "breakfast" not in rec:
            try: await context.bot.send_message(int(sid), "اعلام بیداری نکردیا 😒")
            except Exception as e: logger.warning(f"چک صبحانه دیرکرد {sid}: {e}")
...
def main():
    ...
    jq = app.job_queue
    jq.run_daily(job_breakfast_reminder, time=time(4,15, tzinfo=pytz.utc), name="breakfast")      # ۷:۴۵ تهران
    ...
    jq.run_daily(job_late_breakfast_check, time=time(4, 35, tzinfo=pytz.utc), name="late_breakfast_check")# ۸:۰۵ تهران
