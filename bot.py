...
async def job_backup(context: ContextTypes.DEFAULT_TYPE):
    """ارسال پشتیبان روزانه data.json برای ادمین"""
    try:
        with open(DATA_FILE, "rb") as f:
            await context.bot.send_document(
                ...
                caption=f"💾 پشتیبان روزانه داده‌ها\n📅 {get_today()}"
            )
    except Exception as e:
...
def main():
    ...
    jq.run_daily(job_backup,             time=time(20, 30, tzinfo=pytz.utc), name="backup")  # ۱۲ شب ایران
