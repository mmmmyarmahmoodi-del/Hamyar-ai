...
def main_menu():
    """منوی اصلی با دکمه‌های ثابت پایین صفحه"""
    keyboard = [
...
async def check_membership(user_id: int, bot) -> list:
    """لیست کانال‌هایی که کاربر عضوشون نیست رو برمیگردونه"""
    not_member = []
    for ch in REQUIRED_CHANNELS:
        try:
            ...
            logger.warning(f"خطا در چک عضویت {ch['username']}: {e}")
            # اگه خطا داد (مثلاً ربات ادمین نیست)، بلاک نکن
            # فقط لاگ کن تا بتونی مشکل رو پیدا کنی
    return not_member


def join_keyboard():
    """کیبورد دکمه‌های عضویت در کانال‌ها"""
    buttons = [
...
async def send_join_message(update_or_query, not_joined: list):
    """پیام درخواست عضویت با دکمه‌ها"""
...
async def handle_check_join(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """وقتی کاربر دکمه 'عضو شدم' رو میزنه"""
    ...
    if not_joined:
        await send_join_message(query, not_joined)
    else:
        await query.edit_message_text(
            "✅ عضویتت تأیید شد!\n\nحالا /start رو بزن تا ثبت‌نام کنی."
        )
...
async def register_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # اگه کاربر دکمه‌های منو رو زد، نادیده بگیر
    if full_name in ["📸 ارسال عکس صبحانه", "📝 ارسال گزارش شب"]:
        await update.message.reply_text("لطفاً اول اسم و فامیلت رو بنویس:")
        return REG_NAME
    ...
    await update.message.reply_text(
        ...
        "📝 هر شب قبل از ۱۲ شب گزارش روزانه‌ات رو بفرست\n\n"
        "از دکمه‌های پایین استفاده کن 👇",
        reply_markup=main_menu()
    )
...
async def btn_breakfast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """وقتی دکمه ارسال عکس صبحانه زده میشه"""
    user = update.effective_user
    ...
    await update.message.reply_text(
        "📸 عکس صبحانه‌ات رو همین الان بفرست 👇",
        reply_markup=main_menu()
    )
    context.user_data["waiting_breakfast"] = True
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    save_data(data)
    context.user_data["waiting_breakfast"] = False
...
def main():
    ...
    # گزارش شب - از دکمه منو یا دستور /report
    report_conv = ConversationHandler(
        entry_points=[
            CommandHandler("report", report_start),
            MessageHandler(filters.Text(["📝 ارسال گزارش شب"]), report_start),
        ],
        states={
    ...
    # دکمه ارسال عکس صبحانه از منو
    app.add_handler(MessageHandler(filters.Text(["📸 ارسال عکس صبحانه"]), btn_breakfast))

    # عکس صبحانه (ارسال مستقیم عکس)
